...
pub async fn login_handler(State(state): State<AppState>) -> Result<ApiRedirect, ApiError> {
    ...
    if let Err(err) = state.auth.issue_state(oauth_state.clone()).await {
        error!(target: "oakmoss:api", ?err, "failed to store oauth state");
        return Err(ApiError::internal(err));
    }

    let url = state.auth.authorization_url(&oauth_state).map_err(ApiError::internal)?;
...
pub async fn callback_handler(
    ...
    if !state.auth.consume_state(&query.state).await.map_err(ApiError::internal)? {
        return Err(ApiError::unauthorized("expired oauth state"));
    }

    let tokens = state.auth.exchange_code(&query.code).await.map_err(ApiError::internal)?;
    let user = state.auth.fetch_user(&tokens.access_token).await.map_err(ApiError::internal)?;
...
pub async fn ensure_guild_admin(
    ...
    let perms = member.permissions.and_then(|p| p.parse::<u64>().ok()).unwrap_or_default();
