...
pub async fn create_token_handler(
    ...
    let token =
        state.tokens.create_token(&user_id, body.label).await.map_err(ApiError::internal)?;
...
pub async fn list_tokens_handler(
    ...
    let IdentityContext { user_id, .. } = require_identity(&state, &headers).await?;
    let tokens = state.tokens.list_tokens(&user_id).await.map_err(ApiError::internal)?;
    Ok(ApiJson(Json(tokens.into_iter().map(TokenResponse::from).collect())))
}
...
pub async fn delete_token_handler(
    ...
    let IdentityContext { user_id, .. } = require_identity(&state, &headers).await?;
    let deleted =
        state.tokens.delete_token(&user_id, &token_id).await.map_err(ApiError::internal)?;
    if deleted { Ok(ApiJson(Json(()))) } else { Err(ApiError::not_found("token not found")) }
}
