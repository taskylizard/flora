...
pub fn detect_typescript(source: &str) -> bool {
    ...
    impl<'a> Visit<'a> for TypeScriptDetector {
        ...
        fn visit_ts_interface_declaration(&mut self, _decl: &oxc::ast::ast::TSInterfaceDeclaration<'a>) {
            self.has_typescript = true;
        }
        
        fn visit_ts_type_alias_declaration(&mut self, _decl: &oxc::ast::ast::TSTypeAliasDeclaration<'a>) {
            self.has_typescript = true;
        }
