...
impl AuthService {
    ...
    pub async fn fetch_guild_member(
        ...
        match res.status().as_u16() {
            ...
            other => {
                let body = res.text().await.unwrap_or_default();
                warn!(target: "oakmoss:auth", guild_id, status = other, body, "unexpected guild member response");
                Ok(None)
            }
        }
    }
    ...
    pub async fn get_session(&self, token: &str) -> Result<Option<Session>> {
        let session_id = match self.verify_session_token(token) {
            Ok(id) => id,
            Err(err) => {
                warn!(target: "oakmoss:auth", ?err, "invalid session signature");
                return Ok(None);
            }
        };
        ...
        if session.expires_at <= Utc::now() {
            if let Some(refresh) = session.refresh_token.clone() {
                match self.refresh_token(&refresh).await {
                    ...
                    Err(err) => {
                        warn!(target: "oakmoss:auth", ?err, "failed to refresh expired session");
